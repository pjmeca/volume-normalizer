...
ARG VERSION=3.5.1 \
    ARCH=amd64
RUN apt-get update && \
    apt-get install -y --no-install-recommends curl ca-certificates openssl cron procps && \
...
HEALTHCHECK --interval=60s --timeout=3s --retries=3 CMD pgrep rsgain || /usr/bin/rsgain -v
